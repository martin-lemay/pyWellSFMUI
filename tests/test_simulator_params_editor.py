...
def editor(
    ...
    """Return a SimulatorParamsEditor instance."""
    return SimulatorParamsEditor(state=state, actions=actions)


def test_panel_returns_card(
    editor: SimulatorParamsEditor,
) -> None:
    """Test panel returns a Card widget."""
    result = editor.panel()
    assert isinstance(result, pn.Card)


def test_defaults_pushed_on_init(
    state: AppState,
    editor: SimulatorParamsEditor,
) -> None:
    """Editor pushes default params to state on init."""
    p = state.simulator_params
    assert p is not None
    defaults = FSSimulatorParameters()
    assert (
        p.max_waterDepth_change_per_step
        == defaults.max_waterDepth_change_per_step
    )
...
def test_widget_change_updates_state(
    state: AppState,
    editor: SimulatorParamsEditor,
) -> None:
    """Changing a widget pushes updated params."""
    editor._dt_max.value = 0.5
    assert state.simulator_params.dt_max == 0.5


def test_state_change_updates_widgets(
    state: AppState,
    actions: Actions,
    editor: SimulatorParamsEditor,
) -> None:
    """Programmatic state change syncs widgets."""
